import { start, komenciContextSelector, disableKomenci } from 'src/verify/reducer'
...
function* startVerification(action: typeof start) {
  // Checking Komenci readiness
  const contractKit = yield call(getContractKit)
  const walletAddress = yield call(getConnectedUnlockedAccount)
  Logger.debug(TAG, '@startVerification', walletAddress)
  ...
  try {
    ...
    Logger.error(TAG, '@startVerification', e)
...
export function* verifySaga() {
  ...
  yield takeEvery(start.type, startVerification)
}
